...
app.post('/mapeo/addnewmap', async (req, res) => {
    const{idConnector,idConnFields,idMapp,idTemplate,idTemplateFields,DefaultValue}=req.body
    await addnewmap(idConnector,idConnFields,idMapp,idTemplate,idTemplateFields,DefaultValue)
    res.send('edited')
})

const addnewmap = async (idConnector,idConnFields,idMapp,idTemplate,idTemplateFields,DefaultValue) => {
    if(DefaultValue === ""){
        DefaultValue = null;
    } 
    try {
        const result = await pool.query(`INSERT INTO public."IntegrationMappDef" ("idMapp", "idConnector", "idConnFields", "idTemplate", "idTemplateFields", "DefaultValue") VALUES(${idMapp}, ${idConnector}, ${idConnFields}, ${idTemplate}, ${idTemplateFields}, ${DefaultValue})`)
        return result.rows
    } catch (error) {
        console.log(error)
    }
...
app.get('/mapeo/templates/max', async (req, res) => {
    try {
        ...
        res.json({'lastId':result.rows[0].max})
        return result.rows
    } catch (error) {
        console.log(error)
    }
...
app.get('/mapeo/selectedtemplate/:id', async (req, res) => {
    const mapeo = await getTemplateValue(req.params.id)
    res.send(mapeo)
})

app.get('/mapeo/idtemplate/:id', async (req, res) => {
    const mapeo = await getSelectedtemplate(req.params.id)
    res.send(mapeo)
})

const getTemplateValue = async (id) => {
    try {
        const result = await pool.query(`SELECT "idTemplate" from "IntegrationMappHeader" WHERE "id"=${id}`)
        return result.rows
    } catch (error) {
        console.log(error)
    }
