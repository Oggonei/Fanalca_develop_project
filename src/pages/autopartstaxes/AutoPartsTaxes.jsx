...
import PopupAutoPartsTaxes from "../../components/popupAutopartsTaxes/PopupAutoPartsTaxes.jsx";
import EditPopupAutoPartsTaxes from "../../components/editPopupAutoPartsTaxes/EditPopupAutoPartsTaxes.jsx";

const AutoPartsTaxes = () => {
  let host = window.location.hostname

    const url = `http://${host}:3000/autopartstaxes`;
    ...
    const editData = (value) => {
      ...
      newData.forEach(item => {
        if(item.id === value.id) {
          item.id = value.id,
          item.tax = value.tax,
          item.tipo_iva_iva = value.tipo_iva_iva,
          item.clase_iva_iva =value.clase_iva_iva ,
          item.valor_iva_iva = value.valor_iva_iva ,
          item.tipo_rta_rta = value.tipo_rta_rta,
          item.clase_rta_rta = value.clase_rta_rta,
          item.valor_rta_rta = value.valor_rta_rta ,
          item.tipo_rta_rta_2_5 = value.tipo_rta_rta_2_5 ,
          item.clase_rta_rta_2_5 = value.clase_rta_rta_2_5 ,
          item.valor_rta_rta_2_5 = value.valor_rta_rta_2_5 ,
          item.tipo_rta_iva_15 = value.tipo_rta_iva_15 ,
          item.clase_rta_iva_15 = value.clase_rta_iva_15 ,
          item.valor_rta_iva_15 = value.valor_rta_iva_15 ,
          item.tipo_iva_rta_ser_4 = value.tipo_iva_rta_ser_4 ,
          item.clase_iva_rta_ser_4 = value.clase_iva_rta_ser_4 ,
          item.valor_iva_rta_ser_4 = value.valor_iva_rta_ser_4,
          item.tipo_auto_rta = value.tipo_auto_rta ,
          item.clase_auto_rta = value.clase_auto_rta ,
          item.valor_auto_rta = value.valor_auto_rta 
        }
      })
      setData(newData);
    ...
    const addTemplate = (value) => {
      axios.post(`http://${host}:3000/autopartstaxes`, value)
    ...
    const editTemplate = (itemId,value) => {
      ...
      axios.patch(`http://${host}:3000/autopartstaxes`, newValue)
      .then(function (response) {
        console.log(response);
        fetchData();
      })
    ...
    const popupDeleteItem = () => {
      axios.delete(`http://${host}:3000/autopartstaxes/${itemId}`)
      .then(function (response) {
        console.log(response);
        setTimeout(() => {
          fetchData();
    ...
    return (
      <div className="w-full h-full bg-gradient-to-b from-cyan-600 to-blue-900 lg:h-screen md:h-screen  text-white portrait:h-screen max-w-800px overflow-auto">
        {popup && <PopupAutoPartsTaxes popup={handlePopup} addData={addData} />}
        {editPopup && <EditPopupAutoPartsTaxes popup={handleEditPopup} editData={editData} itemId={data.filter(element => element.id === itemId)} />}
        <Header />
